...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("silent", "Choose if CAS should send DENM or not", silentCAS);
...
YansWifiPhyHelper wifiPhy;
  ...
  AppSampleHelper.SetAttribute ("Silent", BooleanValue (silentCAS));
  ...
  AppSampleHelper.SetAttribute ("Speed", DoubleValue(speed/3.6));
  AppSampleHelper.SetAttribute ("T2C", DoubleValue(t2c_threshold));
  AppSampleHelper.SetAttribute ("Evasive-meneuver", BooleanValue(evasive_maneuver));
  AppSampleHelper.SetAttribute ("Time-val", DoubleValue(time_val));
